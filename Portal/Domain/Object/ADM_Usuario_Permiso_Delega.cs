...
/            Con el ADM_Usuario_Permiso_Delega
/********************************************
/ Fecha : 20-07-2012
/
*********************************************/
...
namespace Portal.Domain.Object
{
    ...
    [Serializable]
    public class ADM_Usuario_Permiso_Delega : IBase
    {
        #region[Propiedades]
            public long IdDelegado{ get; set; }
            public long IdUsuario{ get; set; }
            public long IdUsuarioDelegado{ get; set; }
            public long IdPermiso{ get; set; }
            #endregion
            #region [Constructor]
            public ADM_Usuario_Permiso_Delega()
            {
                Clean();
                CargarTabla();
            }
            public ADM_Usuario_Permiso_Delega(Int64 IdDelegado)
            {
                Clean();
                CargarADM_Usuario_Permiso_Delega(ADM_Usuario_Permiso_DelegaPersistence.CargarDatosADM_Usuario_Permiso_Delega(IdDelegado));
            }
            #endregion
 ...
 sNombreTabla = "Editar ADM_Usuario_Permiso_Delega";
            			iAncho = 700;
            			iAlto = 500;
            			iNumeroPaginas = 10;
            			mTipoFiltro = eTipoFiltro.Simple;            
 sPrimaryKey = new string[] { "IdDelegado" };
                        ...
                        mAnchoColumnas.Add("IdDelegado", 100);
                        mListadoColumnasNombre.Add("IdDelegado", "IdDelegado");
                        mListadoColumnasTipo.Add("IdDelegado", eTipoCampoTabla.Int64);
                        ...
                        mAnchoColumnas.Add("IdUsuarioDelegado", 100);
                        mListadoColumnasNombre.Add("IdUsuarioDelegado", "IdUsuarioDelegado");
                        mListadoColumnasTipo.Add("IdUsuarioDelegado", eTipoCampoTabla.Int64);
            ...
            sProcedimientoGrilla = "ADM_Usuario_Permiso_Delega_Listar";
        ...
        { 
            ...
            private void CargarADM_Usuario_Permiso_Delega(DataRow mRow)
            {
                if (mRow != null)
                {
                     if (mRow["IdDelegado"] != DBNull.Value) IdDelegado = Convert.ToInt64(mRow["IdDelegado"]);
                     if (mRow["IdUsuario"] != DBNull.Value) IdUsuario = Convert.ToInt64(mRow["IdUsuario"]);
                     if (mRow["IdUsuarioDelegado"] != DBNull.Value) IdUsuarioDelegado = Convert.ToInt64(mRow["IdUsuarioDelegado"]);
            ...
            {   //Dictionary<string,long> dModificarDatos = new   Dictionary<string,long>(); //util para obtener llaves introducidas 
                ...
                mParametros.Agregar("@IdUsuarioDelegado",IdUsuarioDelegado);
                mParametros.Agregar("@IdPermiso",IdPermiso);
                lRespuesta = ADM_Usuario_Permiso_DelegaPersistence.ModificarADM_Usuario_Permiso_Delega(mParametros);
        IdDelegado = lRespuesta;
 ...
 mParametros.Agregar("@IdDelegado", IdDelegado);
                ADM_Usuario_Permiso_DelegaPersistence.EliminarADM_Usuario_Permiso_Delega(mParametros);
            } 
            #endregion
    }
}